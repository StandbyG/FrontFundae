...
@Component({
  ...
  styles: [
   ...
   ============================================ */
      ...
      /* Alertas Section - Empleadores sin Ajustes */
      .alertas-section {
        margin-bottom: var(--section-spacing);
      }
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private loadAdminData(): void {
    forkJoin({
      ajustes: this.ajusteService.getAllAjustes(),
      ...
      .subscribe({
        next: (response) => {
          // Log para debug: ver cuántos ajustes se reciben del backend
          console.log(
            '✅ Ajustes recibidos del backend:',
            response.ajustes.length
          );
          console.log('📋 Lista completa de ajustes:', response.ajustes);

          // Asegurar que todos los ajustes se asignen
          this.ajustes = response.ajustes || [];

          // Inicializar filteredAjustes con todos los ajustes
          // Luego aplicar el filtro actual si es necesario
          ...
          console.log(
            '📊 Ajustes después de aplicar filtros:',
            this.filteredAjustes.length
          );

          this.calculateAdminStats();
          this.detectarEmpleadoresSinAjustes(
            response.usuarios,
            response.ajustes
          );
  ...
  private detectarEmpleadoresSinAjustes(
    usuarios: Usuario[],
    ajustes: AjusteRazonable[]
  ): void {
    // Filtrar solo empleadores
    const empleadores = usuarios.filter(
      (usuario) =>
        usuario.tipoUsuario?.toLowerCase() === 'empleador' && usuario.idUsuario
    );

    // Obtener IDs de usuarios que tienen ajustes
    const usuariosConAjustes = new Set<number>();
    ajustes.forEach((ajuste) => {
      if (ajuste.usuario?.idUsuario) {
        usuariosConAjustes.add(ajuste.usuario.idUsuario);
      }
    });

    // Filtrar empleadores que NO tienen ajustes
    this.empleadoresSinAjustes = empleadores.filter(
      (empleador) =>
        empleador.idUsuario && !usuariosConAjustes.has(empleador.idUsuario)
    );

    console.log(
      'Empleadores sin ajustes detectados:',
      this.empleadoresSinAjustes.length
    );
  }
  ...
  private performSearch(searchTerm: string): void {
    // Empezar con TODOS los ajustes, no con filteredAjustes
    ...
    if (term) {
      filtered = filtered.filter((ajuste) => {
        ...
        const nombreEmpresa =
          ajuste.usuario?.nombreEmpresa?.toLowerCase() || '';
    ...
    // Log para debug
    console.log(
      `🔍 Filtro aplicado: "${this.selectedFilter}", Búsqueda: "${searchTerm}"`
    );
    console.log(
      `📈 Total ajustes: ${this.ajustes.length}, Ajustes filtrados: ${this.filteredAjustes.length}`
    );
  }
